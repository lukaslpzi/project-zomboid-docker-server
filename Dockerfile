...
RUN ln -s "${HOMEDIR}/Zomboid/Saves" "${HOMEDIR}/server-saves"

RUN mkdir -p "${HOMEDIR}/Zomboid/Server/" && chown steam:steam "${HOMEDIR}/Zomboid/Server/"
RUN ln -s "${HOMEDIR}/Zomboid/Server" "${HOMEDIR}/server-conf"

WORKDIR ${HOMEDIR}

VOLUME "${HOMEDIR}/server-conf"
VOLUME "${HOMEDIR}/server-saves"
